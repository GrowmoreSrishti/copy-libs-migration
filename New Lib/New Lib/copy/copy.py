...
from dunetuf.udw.udw import Underware, get_underware_instance
...
from dunetuf.job._job_capabilities.job_capabilities import JobCapabilities
...
# TODO: Decouple the Job object usage from this class
class Copy(AbstractJobActions):
    ...
    def __new__(cls: Type[Self], *args: Any, **kwargs: Any) -> Self:
        ...
        if cls is Copy:
            ip_address = get_ip()
            cls._udw = get_underware_instance(ip = ip_address)
            ...
            from dunetuf.copy.dune.copy_dune import \
                CopyDune
            return cast(Self, CopyDune(*args, **kwargs))
        
        return super().__new__(cls)
    
    def __init__(self, cdm: Optional[CDM] = None, udw: Optional[Underware] = None):
        """
        ...
        Args:
            ...
            udw: Optional Underware instance. If not provided, one will be created automatically.
        """
        # Get the device IP address
        ip_address = get_ip()
        
        # Initialize underware and CDM interfaces, using provided instances or creating new ones
        self._udw = udw if udw is not None else get_underware_instance(ip=ip_address)
        ...
        self._job_capabilities = JobCapabilities()
    ...
    def create_ticket(self, payload: Dict = {}, passwords: str = '', headers=None) -> str:
        ...
        """
        # If payload is provided, update internal state based on its contents
        if payload:
        ...
        # Only update the ticket if there are settings to apply
        if payload:
    ...
    def update_ticket(self, ticketId: str, payload: Dict, update_expected: bool = True) -> int:
        """
        Updates a copy job ticket with the provided settings.
        Args:
            ticketId (str): The ticket id to be updated.
            payload (Dict): Dictionary variable to set various copy job parameters.
            update_expected: If True, the update is expected to fail.
        Returns:
            int: Status code of the response
        """
        return self._job_ticket.update(ticketId, payload, update_expected)

    def clone_ticket(self, payload:Dict) -> str:
        """
        Clone a job ticket from the given job information (from job queue or job history).

        Args:
            payload (Dict): The dictionary containing the job information to be cloned.

        Returns:
            str: ticketId of the newly created job ticket
        """
        return self._job_ticket.clone(payload)

    def get_ticket_info(self, ticketId: str) -> Dict:
        """
        Get the copy job ticket information.
        Args:
            ticketId (str): The ticket id to be retrieved.
        Returns:
            Dict: The job ticket information.
        """
        return self._job_ticket.get_info(ticketId)
    ...
    def get_job_stat_info(self, jobid: str, header = None) -> Dict:
        """
        Connects to printer and retrieves specific job stats details based on jobID.
        Args:
            jobid: Job identifier, can take UDW (int) or CDM (str) representation.
        Returns:
            Dict containing job stats.
        """
        return self._job_manager.get_job_stat_info(jobid, header)
    ...
    def get_default_constraints(self) -> Dict:
        """
        Get the default constraints for copy.
        Args:
            None
        Returns:
            Dict containing the default constraints for job tickets by type.
        """
        return  self._cdm.get(self._cdm.JOB_TICKET_COPY_CONSTRAINTS)
    ...
    def update_default_ticket(self, payload: Dict) -> int:
        ...
        Returns:
            int: Status code of the update operation. 200 for success operation.
        """
        print("updating default ticket")
    ...
    def _updating_ticket(self, payload: Dict) -> Dict:
        ...
        """ 
        self._adf_loaded = True
        self._output_duplex = False
    ...
    def get_copy_configuration_constraints(self) -> Dict:
        """
        Get copy configuration constraints
        Returns:
            Dict containing the copy configuration constraints
        """
        return self._cdm.get(self._cdm.COPY_CONFIGURATION_CONSTRAINTS_ENDPOINT)

    def get_job_concurrency_supported(self) -> str:
        """
        Return whether job concurrency is supported.

        Returns:
            A string indicating whether job concurrency is supported.
        """
        return self._job_capabilities.get_job_concurrency_supported()

    def get_constraints(self, ticketId: str) -> Dict:
        """
        Get constraints for a specific job ticket.

        Args:
            ticketId: Job's ticket id

        Returns:
            Dict: containing the job's ticket constraints
        """
        return self._job_ticket.get_constraints(ticketId)

    def wait_for_alerts(self, category: str = "any") -> List[Dict[str, Any]]:
        """
        Wait until a specific category of media alert is observed.
        Args:
            category (str): The category of alerts to filter by. Defaults to 'any'.
        Returns:
            List[Dict[str, Any]]: A list of alerts matching the specified category.
        Raises:
            TimeoutError if no alerts of the specified category are observed within the timeout.
        """
        return self._job_manager.wait_for_alerts(category)

    def alert_action(self, category: str, response: str) -> None:
        """
        Perform an action on alerts of a specific category.
        Args:
            category (str): The category of alerts to filter by.
            response (str): The response action to perform on the alert.
        Raises:
            AssertionError if the response status code is not 200.
        """
        return self._job_manager.alert_action(category, response)
                ...
                if (self.has_two_segment_pipeline(ticket_id)):
                    ...
                    if cancel == Cancel.after_start:
                        ...
                        if cancel == Cancel.submit_and_exit: #reusing cancel parameter to submit job and exit without waiting for completion
                            ...
                            logging.info('About to wait for job completion. CDM Job ID: %s', job_id)
                            # Convert to UDW format for debugging
                            udw_job_id = self._job.get_jobid(job_id, guid=False)
                            logging.info('UDW Job ID will be: %s', udw_job_id)
                            self._job.wait_for_job_state(udw_job_id, state='COMPLETED', timeout=max_wait_time)
    ...
    def validate_settings_used_in_copy(self, original_size=None, paper_size=None, lighter_darker=None,
                                        ...
                                        multiple_feed_detect=None, multiple_feed_auto_retry=None,
                                        color_mode=None, tray_setting=None, sides=None, orientation=None,
                                        quality=None, copy_margins=None,content_type=None, two_side_page_flip_up=None, media_type=None,
                                        ...
                                        stamp_text_size=None, stamp_starting_page=None, stamp_starting_number=None, stamp_num_of_digit=None, stamp_page_numbering=None, stamp_white_background=None, sharpness=None, contrast=None, background_cleanup=None, auto_paper_color_removal=None, outputBin=None) -> None:
        """
        ...
        :param multiple_feed_detect:
        :param multiple_feed_auto_retry:
        :param collate:
        :param media_source:
        ...
        :param sharpness:
        :param contrast:
        :param background_cleanup:
        :param auto_paper_color_removal:
        :param outputBin:
        """
        ...
        if multiple_feed_detect:
            logging.info("Check the multiple feed detect")
            assert copy_job_details["src"]["scan"]["multipleFeedDetect"] == multiple_feed_detect, "Wrong multiple feed detect"

        if multiple_feed_auto_retry:
            logging.info("Check the multiple feed auto retry")
            assert copy_job_details["src"]["scan"]["multipleFeedAutoRetry"] == multiple_feed_auto_retry, "Wrong multiple feed auto retry"
        ...
        if media_type:
            logging.info("Check media type")
            assert copy_job_details["dest"]["print"]["mediaType"] == media_type, "Wrong media type"
        ...
        if sharpness:
            logging.info("Check sharpness Setting")
            assert copy_job_details["pipelineOptions"]["imageModifications"]["sharpness"] == sharpness, "Wrong sharpness setting"

        if contrast:
            logging.info("Check contrast Setting")
            assert copy_job_details["pipelineOptions"]["imageModifications"]["contrast"] == contrast, "Wrong contrast setting"

        if background_cleanup:
            logging.info("Check background cleanup Setting")
            assert copy_job_details["pipelineOptions"]["imageModifications"]["backgroundCleanup"] == background_cleanup, "Wrong background cleanup setting"

        if auto_paper_color_removal:    
            logging.info("Check auto paper color removal Setting")
            assert copy_job_details["pipelineOptions"]["imageModifications"]["autoPaperColorRemoval"] == auto_paper_color_removal, "Wrong auto paper color removal setting"

        if outputBin:
            logging.info("Check output bin Setting")
            assert copy_job_details["dest"]["print"]["mediaDestination"] == outputBin, "Wrong output bin setting"          
